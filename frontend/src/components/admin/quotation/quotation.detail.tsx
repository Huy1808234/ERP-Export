...
const QuotationDetailModal = ({ quotationId, open, onClose, onSuccess }: IProps) => {
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      const user = session?.user as ISessionUser | undefined;
  ...
  const lineColumns: ColumnsType<IQuotationLine> = useMemo(() => [
    { title: 'STT', render: (_, __, i) => i + 1, width: 50 },
    {
      ...
      render: (_, record) => (
        <div>
          ...
          {record.product?.sku && <div style={{ fontSize: 11, color: '#94a3b8' }}>SKU: {record.product.sku}</div>}
        </div>
      ),
    },
    { 
    ...
    },
    {
      ...
      render: (value: number, record) => {
        ...
        return <Text strong style={{ color: '#1677ff' }}>{formatCurrency(amount, 2)}</Text>;
      },
    },
  ], [data?.currency]);
  ...
  return (
    <Modal
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingRight: 24 }}>
          <Space size="middle">
            ...
            <div>
              ...
              <Text style={{ fontSize: 13, color: '#1890ff', fontWeight: 500 }}>{data?.quotationNumber || 'Đang tải...'}</Text>
            </div>
            {data?.status && (
              <Tag 
                variant="filled"
      ...
      styles={{ 
        ...
        header: { padding: '20px 24px', borderBottom: '1px solid rgba(0,0,0,0.06)', margin: 0, background: '#fff' }
      }}
      footer={
        <div style={{ padding: '16px 24px', background: '#fff', borderTop: '1px solid rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
             <Space size="middle">
                <span style={{ color: '#64748b', fontSize: 13, fontWeight: 500 }}>Trạng thái:</span>
            ...
            <Space size="middle">
              ...
              <Button 
                ...
                style={{ borderRadius: 8, height: 40, boxShadow: '0 4px 12px rgba(24, 144, 255, 0.3)' }} 
                onClick={handlePrint}
              >
                Xuất PDF / In
              </Button>
              <Button 
                ...
                style={{ color: '#52c41a', borderColor: '#52c41a', borderRadius: 8, height: 40 }}
          ...
          <div style={{ maxHeight: 'calc(85vh - 120px)', overflowY: 'auto', padding: '24px 32px' }}>
            <div ref={printRef}>
              <div style={{ 
                ...
                <div>
                  <div style={{ fontSize: 12, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Khách hàng</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#1e293b' }}>{data.customer?.name}</div>
                  <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>{data.customer?.address}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Thông tin chung</div>
                ...
                <div>
                  <div style={{ fontSize: 12, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Logistics & Thanh toán</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 4, height: 18, background: '#1890ff', borderRadius: 2 }}></div>
              ...
              <Table<IQuotationLine>
                ...
                columns={lineColumns}
                rowKey={(r) => r.id || `${r.product?.id || Math.random()}`}
                pagination={false}
                style={{ 
                  borderRadius: 12, 
                  overflow: 'hidden',
                                ...
                                <div style={{ padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                                    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 40 }}>
                                        ...
                                        <Text strong style={{ fontSize: 14, width: 180, textAlign: 'right' }}>
                                            {data.currency} {formatCurrency((Number(data.totalAmount || 0) - Number(data.logisticsFee || 0) - Number(data.otherFee || 0)), 2)}
                                        </Text>
                                    </div>
                                    {(Number(data.logisticsFee) > 0) && (
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 40 }}>
                                            <Text type="secondary" style={{ fontSize: 13 }}>Phí Logistics:</Text>
                                            <Text strong style={{ fontSize: 14, width: 180, textAlign: 'right' }}>
                                    ...
                                    <div style={{ 
                                        ...
                                        <Text style={{ fontSize: 16, fontWeight: 700, color: '#1e293b' }}>TỔNG CỘNG:</Text>
                                        <Text style={{ color: '#ff4d4f', fontSize: 24, fontWeight: 800, width: 220, textAlign: 'right' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20, marginBottom: 32 }}>
                <div style={{ background: '#e6f7ff', padding: 20, borderRadius: 16, border: '1px solid #91d5ff', display: 'flex', alignItems: 'center', gap: 16 }}>
                    <BlockOutlined style={{ fontSize: 24, color: '#1890ff' }} />
                    <div>
                        <div style={{ fontSize: 12, color: '#1890ff', fontWeight: 600 }}>Tổng số thùng</div>
                        <div style={{ fontSize: 20, fontWeight: 800 }}>{logisticsSummary.totalCartons.toFixed(1)} <span style={{ fontSize: 12 }}>CTNS</span></div>
                    </div>
                </div>
                <div style={{ background: '#f9f0ff', padding: 20, borderRadius: 16, border: '1px solid #d3adf7', display: 'flex', alignItems: 'center', gap: 16 }}>
                    <DashboardOutlined style={{ fontSize: 24, color: '#722ed1' }} />
                    <div>
                        <div style={{ fontSize: 12, color: '#722ed1', fontWeight: 600 }}>Tổng trọng lượng</div>
                        <div style={{ fontSize: 20, fontWeight: 800 }}>{logisticsSummary.totalGW.toFixed(2)} <span style={{ fontSize: 12 }}>KGS</span></div>
                    </div>
                </div>
                <div style={{ background: '#f6ffed', padding: 20, borderRadius: 16, border: '1px solid #b7eb8f', display: 'flex', alignItems: 'center', gap: 16 }}>
                    <ThunderboltOutlined style={{ fontSize: 24, color: '#52c41a' }} />
                    <div>
                        <div style={{ fontSize: 12, color: '#52c41a', fontWeight: 600 }}>Tổng thể tích</div>
                        <div style={{ fontSize: 20, fontWeight: 800 }}>{logisticsSummary.totalCBM.toFixed(3)} <span style={{ fontSize: 12 }}>M³</span></div>
                    </div>
                </div>
              </div>

              {data.bankInfo && (
                <div style={{ padding: '16px 24px', background: '#f8fafc', borderRadius: 12, borderLeft: '4px solid #3b82f6', marginBottom: 16 }}>
                  <Text strong>Thông tin thanh toán:</Text>
                  <pre style={{ margin: '8px 0 0 0', whiteSpace: 'pre-wrap', fontFamily: 'inherit', fontSize: 13 }}>{data.bankInfo}</pre>
                </div>
              )}

              {data.note && (
                <div style={{ padding: '16px 24px', background: '#fffbe6', borderRadius: 12, borderLeft: '4px solid #ffe58f' }}>
                  <Text strong>Ghi chú:</Text> {data.note}
                </div>
              )}
            </div>
          </div>
