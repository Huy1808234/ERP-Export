...
import {
  LetterOfCredit,
  LCStatus,
} from '../trade-finance/entities/letter-of-credit.entity';
...
export class DashboardsService {
  ...
  private getUserCacheScope(
    user?: DashboardUser,
  ): Record<string, string | null> {
    const role = user?.role;
    const roleName =
      typeof role === 'string' ? role : role?.name || user?.roleName || null;
  ...
  async getAdminDashboard(
    ...
    const cacheKey = this.cache.makeKey('dashboard:admin', {
      startDate,
      endDate,
      user: this.getUserCacheScope(user),
    });
    ...
    return this.cache.getOrSet(
      ...
      async () => {
        const executive = await this.getExecutiveDashboard(
          startDate,
          endDate,
          user,
        );
        ...
        return maskCostFields(
          {
            ...
            finance: {
              ...
              exchangeRate,
              history,
              logisticsCostBreakdown: await this.getLogisticsCostBreakdown(
                start,
                end,
                exchangeRate,
              ),
  ...
  private toVnd(
    ...
    return (currency || 'VND').toUpperCase() === 'VND'
      ? value
      : value * exchangeRate;
  }
  ...
  private buildMarginLine(
    ...
    const grossProfitMarginPercent =
      revenueVnd > 0 ? (grossProfitVnd / revenueVnd) * 100 : 0;
  ...
  private async getLogisticsCostBreakdown(
    start: Date,
    end: Date,
    exchangeRate: number,
  ) {
    ...
    const totals = shipments.reduce(
      (acc, shipment) => {
        acc.seaFreight += this.toVnd(
          shipment.freightCost,
          shipment.freightCurrency,
          exchangeRate,
        );
        acc.insurance += this.toVnd(
  ...
  async getKpiDrilldown(
    ...
    endDate?: string,
    user?: DashboardUser,
  ): Promise<DashboardKpiDrilldown> {
    const cacheKey = this.cache.makeKey('dashboard:kpi-drilldown', {
      startDate,
      endDate,
      user: this.getUserCacheScope(user),
    });
    ...
    return this.cache.getOrSet(
      ...
      async () => {
        ...
        const productRows = await this.scRepository
          ...
          .addSelect(
            'COALESCE(product.vietnameseName, product.englishName, product.sku)',
            'productName',
          )
          .addSelect('COALESCE(product.category, :uncategorized)', 'category')
          .addSelect('SUM(CAST(item.quantity AS NUMERIC))', 'quantity')
          .addSelect(
            'SUM(CAST(item.totalPrice AS NUMERIC) * CAST(COALESCE(sc.exchangeRate, 1) AS NUMERIC))',
            'revenueVnd',
          )
          .addSelect(
            'SUM(CAST(item.quantity AS NUMERIC) * CAST(COALESCE(product.purchasePriceVnd, 0) AS NUMERIC))',
            'cogsVnd',
          )
          .addSelect(
            'MAX(CAST(COALESCE(product.currentStock, 0) AS NUMERIC))',
            'currentStock',
          )
          .addSelect(
            'MAX(CAST(COALESCE(product.purchasePriceVnd, 0) AS NUMERIC))',
            'purchasePriceVnd',
          )
          .where('sc.status != :cancelled', {
            cancelled: SalesContractStatus.CANCELLED,
          })
        ...
        const marketRows = await this.scRepository
          ...
          .select(
            "COALESCE(NULLIF(buyer.region::text, ''), NULLIF(buyer.country, ''), :unknownMarket)",
            'market',
          )
          .addSelect(
            'SUM(CAST(item.totalPrice AS NUMERIC) * CAST(COALESCE(sc.exchangeRate, 1) AS NUMERIC))',
        ...
        const openReceivableVnd =
          await this.accountingService.getAccountBalance('131');
        const today = dayjs().startOf('day').toDate();
        const arOpenStatuses = [
          ARStatus.UNPAID,
          ARStatus.PARTIAL,
          ARStatus.OVERDUE,
        ];
        ...
        const overdueSummary = await this.arRepository
          ...
          .addSelect(
            'SUM(GREATEST(CAST(ar.amountVnd AS NUMERIC) - CAST(ar.paidAmountVnd AS NUMERIC), 0))',
            'amountVnd',
          )
        ...
        const inventoryTurnoverByProduct = productRows
          .map<InventoryTurnoverLine>((row) => {
            ...
            const inventoryValueVnd =
              this.parseNumber(row.currentStock) *
              this.parseNumber(row.purchasePriceVnd);
            const turnover =
              inventoryValueVnd > 0 ? cogsVnd / inventoryValueVnd : 0;
        ...
        const costBreakdown = await this.getLogisticsCostBreakdown(
          start,
          end,
          exchangeRate,
        );
        const logisticsCostVnd = costBreakdown.reduce(
          (sum, item) => sum + Number(item.value || 0),
          0,
        );
        const logisticsCostRatioPercent =
          totalRevenueVnd > 0 ? (logisticsCostVnd / totalRevenueVnd) * 100 : 0;
        const shipments = await this.shipmentRepository.find({
          where: { createdAt: Between(start, end) },
        ...
        const dashboard: DashboardKpiDrilldown = {
          ...
          dso: {
            ...
            dsoDays:
              totalRevenueVnd > 0
                ? Number(
                    ((openReceivableVnd / totalRevenueVnd) * days).toFixed(1),
                  )
                : 0,
            ...
            topOverdueInvoices: overdueInvoices.map((item) => ({
              ...
              overdueDays: item.dueDate
                ? Math.max(dayjs(today).diff(dayjs(item.dueDate), 'day'), 0)
                : 0,
              openAmountVnd: Math.max(
                Number(item.amountVnd || 0) - Number(item.paidAmountVnd || 0),
                0,
              ),
              status: item.status,
            })),
          ...
          logisticsRevenue: {
            ...
            logisticsCostVnd,
            logisticsCostRatioPercent: Number(
              logisticsCostRatioPercent.toFixed(2),
            ),
            costBreakdown,
            shipments: shipments.map((shipment) => {
              ...
              const shipmentRevenueVnd = Number(
                shipment.salesContract?.totalAmountVnd || 0,
              );
              ...
              return {
                ...
                logisticsCostRatioPercent:
                  shipmentRevenueVnd > 0
                    ? Number(
                        ((shipmentCostVnd / shipmentRevenueVnd) * 100).toFixed(
                          2,
                        ),
                      )
                    : 0,
  ...
  async getExecutiveDashboard(
    ...
    const cacheKey = this.cache.makeKey('dashboard:executive', {
      startDate,
      endDate,
      user: this.getUserCacheScope(user),
    });
    ...
    return this.cache.getOrSet(
      ...
      async () => {
        ...
        const inventoryData = await this.productRepository
          ...
          .select(
            'SUM(CAST(product.currentStock AS NUMERIC) * CAST(COALESCE(product.purchasePriceVnd, 0) AS NUMERIC))',
            'totalValue',
          )
          .getRawOne();
        ...
        const salesStats = {
          ...
          confirmedContracts: await this.scRepository.count({
            where: { status: SalesContractStatus.CONFIRMED },
          }),
          shippedPIs: await this.scRepository.count({
            where: { status: SalesContractStatus.SHIPPED },
          }),
          pendingShipments: await this.scRepository.count({
            where: { status: SalesContractStatus.CONFIRMED },
          }), // Đã chốt nhưng chưa ship
          conversionRate: 0,
        };
        if (salesStats.totalPIs > 0)
        ...
        const upcomingShipments = await this.shipmentRepository.find({
          where: [
            {
              etd: Between(new Date(), next14Days),
              status: Not(ShipmentStatus.CLOSED),
            },
            {
              eta: Between(new Date(), next14Days),
              status: Not(ShipmentStatus.CLOSED),
            },
        ...
        const shipmentRowsWithEtd = completedShipmentRows.filter(
          (shipment) => shipment.etd,
        );
        const onTimeShipments = shipmentRowsWithEtd.filter((shipment) => {
          ...
          return (
            actualMilestone &&
            shipment.etd &&
            new Date(actualMilestone) <= new Date(shipment.etd)
          );
        }).length;
        const onTimeRate =
          shipmentRowsWithEtd.length > 0
            ? (onTimeShipments / shipmentRowsWithEtd.length) * 100
            : 0;
        ...
        const totalCustomers = await this.partnerRepository.count({
          where: { partnerType: PartnerType.CUSTOMER },
        });
        ...
        const statusBreakdown = liveShipments.reduce<
        ...
        const exchangeRate = 25450;
        const logisticsCostVnd = await this.getLogisticsCostTotal(
          start,
          end,
          exchangeRate,
        );
        ...
        const dashboard = {
          director: {
            ...
            grossProfitMargin:
              summary.current.revenue > 0
                ? (summary.current.netProfit / summary.current.revenue) * 100
                : 0,
            arAging,
          ...
          lowStockProducts: lowStockProducts.map((p) => ({
            ...
            safetyStock: Math.max(this.parseNumber(p.minimumStock), 100),
            imageUrl: null,
          })),
  ...
  private async getLogisticsCostTotal(
    start: Date,
    end: Date,
    exchangeRate: number,
  ) {
    const breakdown = await this.getLogisticsCostBreakdown(
      start,
      end,
      exchangeRate,
    );
  ...
  private async getCashflowForecast(exchangeRate: number) {
    ...
    const inflowVnd = receivables.reduce(
      (sum, item) =>
        sum +
        Math.max(
          Number(item.amountVnd || 0) - Number(item.paidAmountVnd || 0),
    ...
    const outflowVnd = payables.reduce((sum, item) => {
      ...
      return (
        sum +
        ((item.currency || 'VND').toUpperCase() === 'VND'
          ? remaining
      ...
      where: [
        {
          ...
          expiryDate: Between(today, next14Days),
        },
        {
          status: In([
            LCStatus.RECEIVED,
          ...
          latestShipmentDate: Between(today, next14Days),
        },
        {
          status: In([
            LCStatus.RECEIVED,
          ...
          presentationDeadline: Between(today, next14Days),
        },
      ],
      relations: ['salesContract', 'salesContract.buyer'],
  ...
  async getPortalSummary(user: PortalDashboardUser) {
    const cacheKey = this.cache.makeKey('dashboard:portal-summary', {
      user: this.getUserCacheScope(user),
    ...
    return this.cache.getOrSet(
      ...
      async () => {
        ...
        const totalDebt =
          await this.accountingService.getAccountBalanceForPartner(
            '131',
            partnerId,
          );
