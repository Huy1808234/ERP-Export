...
export class LogisticsAllocationService {
   ...
   */
  async allocateCosts(shipmentId: string, method: AllocationMethod = AllocationMethod.BY_CBM): Promise<ShipmentCostAllocation[]> {
    ...
    if (!shipment.salesContract?.items?.length) {
      throw new BadRequestException('Shipment does not have any items to allocate');
    }
    ...
    const freightInsuranceVnd = new Decimal(shipment.freightCost || 0).plus(shipment.insuranceCost || 0).times(exRate);
    ...
    const itemsBasis = items.map(item => {
      ...
      switch (method) {
        ...
        case AllocationMethod.BY_WEIGHT:
          const gw = new Decimal(product?.grossWeightPerCarton || product?.netWeightPerCarton || 0);
          const pcs = new Decimal(product?.piecesPerCarton || 1);
          basisValue = qty.div(pcs).mul(gw);
          break;
        case AllocationMethod.BY_VALUE:
          basisValue = new Decimal(item.totalPrice || qty.mul(item.unitPrice || 0));
          break;
        default: // BY_QUANTITY
          basisValue = qty;
      }
      ...
      if (basisValue.isZero() && method !== AllocationMethod.BY_QUANTITY) basisValue = qty;
      
      totalBasis = totalBasis.plus(basisValue);
      return { productId: item.productId, basisValue };
    });

    if (totalBasis.isZero()) throw new BadRequestException('Total Basis is 0, cannot allocate.');
    ...
    const allocatePart = (totalAmount: Decimal) => {
      ...
      const parts = itemsBasis.map(ib => {
        ...
        return { productId: ib.productId, allocated: floor, remainder: exact.minus(floor) };
      });

      let diff = totalAmount.minus(sumAllocated).toNumber();
      parts.sort((a, b) => b.remainder.minus(a.remainder).toNumber());
      for (let i = 0; i < diff; i++) {
        parts[i % parts.length].allocated = parts[i % parts.length].allocated.plus(1);
      }
      return parts;
    };
    ...
    const allocations = itemsBasis.map(ib => {
      ...
      if (!f || !l) {
        throw new BadRequestException(`Allocation parts not found for product ${ib.productId}`);
      }
