...
import { CollectionOrder, CollectionOrderStatus, CollectionOrderType } from './entities/collection-order.entity';
import {
...
type LCDeadlineType = 'EXPIRY' | 'LATEST_SHIPMENT' | 'PRESENTATION' | 'DISCREPANCY' | 'INVOICE_DUE';
type LCDeadlineSeverity = 'OVERDUE' | 'TODAY' | 'CRITICAL' | 'WARNING' | 'UPCOMING';
...
export class TradeFinanceService extends BaseService<LetterOfCredit> {
  ...
  async createLCDiscrepancy(id: string, dto: CreateLCDiscrepancyDto, user: User) {
    const lc = await this.findOneLC(id);
    if (![LCStatus.DOCUMENTS_PRESENTED, LCStatus.RECEIVED, LCStatus.ACCEPTED].includes(lc.status)) {
      throw new BadRequestException('Chi ghi nhan discrepancy sau khi L/C da nhan hoac da xuat trinh chung tu');
    }
  ...
  async resolveLCDiscrepancy(id: string, discrepancyId: string, dto: ResolveLCDiscrepancyDto, user: User) {
    await this.findOneLC(id);
    ...
    if (![LCDiscrepancyStatus.AMENDED, LCDiscrepancyStatus.WAIVED, LCDiscrepancyStatus.ACCEPTED_BY_BUYER, LCDiscrepancyStatus.RESOLVED, LCDiscrepancyStatus.CANCELLED].includes(dto.status)) {
      throw new BadRequestException('Trang thai xu ly discrepancy khong hop le');
    }

    discrepancy.status = dto.status;
    discrepancy.resolutionNote = dto.resolutionNote || discrepancy.resolutionNote;
  ...
  async getLCAlerts(days = 14): Promise<Record<string, unknown>> {
    ...
    const active = lcs.filter((lc) => ![LCStatus.PAID, LCStatus.CANCELLED, LCStatus.EXPIRED].includes(lc.status));
    const expiring = active.filter((lc) => new Date(lc.expiryDate) <= horizon);
    const shipmentDeadline = active.filter((lc) => lc.latestShipmentDate && new Date(lc.latestShipmentDate) <= horizon);
    const presentationDeadline = active.filter((lc) => lc.presentationDeadline && new Date(lc.presentationDeadline) <= horizon);
    const openDiscrepancies = await this.discrepancyRepository.find({
      where: { status: LCDiscrepancyStatus.OPEN },
      relations: ['letterOfCredit', 'letterOfCredit.salesContract', 'letterOfCredit.salesContract.buyer'],
      order: { dueDate: 'ASC', createdAt: 'DESC' },
    });

    const receivables = await this.findReceivablesForLCs(active);
    const invoiceSnapshotsByContract = this.groupInvoiceSnapshotsByContract(receivables);
    const contractSnapshotById = new Map(active.map((lc) => [lc.salesContractId, lc]));
    const timeline = [
      ...
      ...this.buildInvoiceDeadlineItems(receivables, contractSnapshotById, horizon, now),
    ].sort((left, right) => {
    ...
    const nextActions = timeline
      .filter((item) => ['OVERDUE', 'TODAY', 'CRITICAL', 'WARNING'].includes(item.severity))
      .slice(0, 12);
    ...
    return {
      ...
      notificationChannels: [
        { channel: 'DASHBOARD', enabled: true, description: 'Visible in deadline dashboard and badges' },
        { channel: 'SOCKET', enabled: true, description: 'Realtime in-app broadcast for active admin sessions' },
        { channel: 'EMAIL_DIGEST', enabled: false, description: 'Digest payload is prepared; SMTP routing can be enabled per tenant' },
      ],
      exposure: {
        activeLcAmount: this.sumUniqueLcAmount(active.map((lc) => ({
          lcId: lc._id,
          amount: Number(lc.amount || 0),
        }))),
        deadlineAmount: this.sumUniqueLcAmount(timeline),
        overdueAmount: this.sumUniqueLcAmount(timeline.filter((item) => item.severity === 'OVERDUE')),
        criticalAmount: this.sumUniqueLcAmount(timeline.filter((item) => ['TODAY', 'CRITICAL'].includes(item.severity))),
        presentationAmount: this.sumUniqueLcAmount(timeline.filter((item) => item.type === 'PRESENTATION')),
        discrepancyAmount: this.sumUniqueLcAmount(timeline.filter((item) => item.type === 'DISCREPANCY')),
      ...
      windows: {
        ...
        next7Days: timeline.filter((item) => item.daysRemaining !== null && item.daysRemaining > 0 && item.daysRemaining <= 7),
        next14Days: timeline.filter((item) => item.daysRemaining !== null && item.daysRemaining > 7 && item.daysRemaining <= 14),
      },
      counts: {
  ...
  private async findReceivablesForLCs(lcs: LetterOfCredit[]): Promise<AccountReceivable[]> {
    const contractIds = Array.from(new Set(lcs.map((lc) => lc.salesContractId).filter(Boolean)));
  ...
  private toInvoiceDeadlineSnapshot(receivable: AccountReceivable): LCDeadlineInvoiceSnapshot {
  ...
  private enrichLCDeadlineItemsWithInvoices(
    ...
    return items.map((item) => {
      const invoices = invoiceSnapshotsByContract.get(item.salesContractId) || [];
      return {
        ...item,
        invoices,
        openAmountForeign: invoices.reduce((sum, invoice) => sum + invoice.openAmountForeign, 0),
  ...
  private buildInvoiceDeadlineItems(
    ...
    receivables.forEach((receivable) => {
      if (!receivable.salesContractId || [ARStatus.PAID, ARStatus.CANCELLED].includes(receivable.status)) return;
      if (!receivable.dueDate) return;
      const dueDate = new Date(receivable.dueDate);
      if (Number.isNaN(dueDate.getTime()) || dueDate.getTime() > horizon.getTime()) return;

      const openAmountForeign = Math.max(
        Number(receivable.amountForeign || 0) - Number(receivable.paidAmountForeign || 0),
        0,
      );
  ...
  private buildDeadlineNotifications(items: LCDeadlineItem[]): LCDeadlineNotification[] {
    return items.map((item) => ({
      ...
      channels: item.notificationChannels || this.getNotificationChannels(item.severity),
      lcId: item.lcId,
  ...
  private getNotificationChannels(severity: LCDeadlineSeverity): LCDeadlineNotificationChannel[] {
  ...
  private buildLCDeadlineItems(lcs: LetterOfCredit[], horizon: Date, now: Date): LCDeadlineItem[] {
    const items: LCDeadlineItem[] = [];

    for (const lc of lcs) {
      this.pushLCDeadlineItem(items, lc, 'EXPIRY', 'L/C expiry', lc.expiryDate, horizon, now, 'Close payment or extend L/C before expiry');
      this.pushLCDeadlineItem(items, lc, 'LATEST_SHIPMENT', 'Latest shipment', lc.latestShipmentDate, horizon, now, 'Confirm shipment/export delivery against L/C');
      this.pushLCDeadlineItem(items, lc, 'PRESENTATION', 'Document presentation', lc.presentationDeadline, horizon, now, 'Present export documents to advising bank');
    }
  ...
  private buildLCDiscrepancyDeadlineItems(
    ...
    for (const discrepancy of discrepancies) {
      const dueDate = discrepancy.dueDate ? new Date(discrepancy.dueDate) : null;
      if (dueDate && dueDate.getTime() > horizon.getTime()) continue;

      const lc = discrepancy.letterOfCredit;
      const daysRemaining = dueDate ? this.getDaysRemaining(dueDate, now) : null;
      const severity = daysRemaining === null
  ...
  private getDiscrepancySeverity(severity: LCDiscrepancySeverity): LCDeadlineSeverity {
    if ([LCDiscrepancySeverity.CRITICAL, LCDiscrepancySeverity.HIGH].includes(severity)) {
      return 'CRITICAL';
    }
    return 'WARNING';
  }
  ...
  private buildLCDeadlineGroups(items: LCDeadlineItem[], groupBy: 'CONTRACT' | 'BUYER'): LCDeadlineGroup[] {
    const groups = new Map<string, LCDeadlineGroup>();

    items.forEach((item) => {
      const key = groupBy === 'CONTRACT'
        ? item.salesContractId || item.lcId
        : item.buyerName || 'UNKNOWN_BUYER';
      const label = groupBy === 'CONTRACT'
      ...
      const group: LCDeadlineGroup = groups.get(key) || {
        key,
        label,
        salesContractId: groupBy === 'CONTRACT' ? item.salesContractId : undefined,
        contractNumber: groupBy === 'CONTRACT' ? item.contractNumber : undefined,
      ...
      group.nextDeadline = this.getEarlierDate(group.nextDeadline, item.dueDate);
    ...
    return Array.from(groups.values())
      ...
      .sort((left, right) => {
        ...
        return new Date(left.nextDeadline).getTime() - new Date(right.nextDeadline).getTime();
      });
  }
  ...
  private getWorstSeverity(values: LCDeadlineSeverity[]) {
    return values.sort((left, right) => this.getSeverityRank(left) - this.getSeverityRank(right))[0];
  }
  ...
  private sumUniqueLcAmount(items: Array<{ lcId: string; amount: number }>) {
    ...
    return Array.from(amountByLc.values()).reduce((sum, amount) => sum + amount, 0);
  }
  ...
  private async assertNoBlockingLCDiscrepancy(lcId: string) {
    ...
    if (openCount > 0) {
      throw new BadRequestException('L/C con discrepancy dang mo, khong the chuyen sang ACCEPTED');
    }
  }
  ...
  async findAllCollections(query: QueryParams) {
    ...
    ['current', 'pageSize', 'limit', 'skip'].forEach(key => delete filter[key]);
  ...
  async updateCollectionStatus(id: string, status: CollectionOrderStatus) {
      const collection = await this.collectionRepository.findOne({ where: { _id: id } });
      if (!collection) throw new BadRequestException('Collection Order not found');
  ...
  async getReconciliationSummary(salesContractId: string) {
    ...
    const receivedTransactions = transactions.filter((tx) => tx.status === TradeFinanceStatus.RECEIVED);
    ...
    const receivedCollections = receivedTransactions
      .filter((tx) => [TradeFinanceType.DP, TradeFinanceType.DA].includes(tx.type))
      .reduce((sum, tx) => sum + Number(tx.amount || 0), 0);
    ...
    const arTotal = receivables.reduce((sum, ar) => sum + Number(ar.amountForeign || 0), 0);
    const arPaid = receivables.reduce((sum, ar) => sum + Number(ar.paidAmountForeign || 0), 0);
    const openAr = arTotal > 0 ? Math.max(arTotal - arPaid, 0) : Math.max(contractTotal - receivedTotal, 0);
    ...
    const receivableAllocations = receivables.map((ar) => {
      ...
      allocationDetailsFlat.push(...allocationDetails.map((allocation) => ({
        ...allocation,
      ...
      const advanceAllocated = allocationDetails
        ...
        .reduce((sum, allocation) => sum + allocation.allocatedAmountForeign, 0);
      ...
      return {
        ...
        remainingForeign: Math.max(Number(ar.amountForeign || 0) - Number(ar.paidAmountForeign || 0), 0),
        currency: ar.currency,
        status: ar.status,
    ...
    const invoiceAllocationMatrix = receivableAllocations.map((row) => {
      const allocatedTotal = row.advanceAllocated + row.balanceAllocated + row.collectionAllocated + row.manualAllocated;
      const allocationPercent = row.amountForeign > 0
        ? Number(new Decimal(allocatedTotal).div(row.amountForeign).mul(100).toFixed(2))
        : 0;
      ...
      return {
        ...
        allocatedTotal,
        allocationPercent,
        advanceStatus: row.advanceAllocated > 0
          ? 'ALLOCATED'
          : allocationTotals.ADVANCE < expectedAdvance
            ? 'PENDING_ADVANCE'
            : 'NOT_REQUIRED',
        balanceStatus: row.remainingForeign <= 0
          ? 'SETTLED'
          : row.balanceAllocated + row.collectionAllocated > 0
            ? 'PARTIAL_BALANCE'
            : 'OPEN_BALANCE',
        suggestedNextStage: row.remainingForeign <= 0
          ? 'SETTLED'
          : allocationTotals.ADVANCE < expectedAdvance
            ? 'ADVANCE'
            : 'BALANCE',
    ...
    const transactionAllocations = receivedTransactions.map((tx) => {
      const allocations = allocationDetailsFlat.filter((allocation) => allocation.tradeFinanceTransactionId === tx._id);
      const allocatedAmount = allocations.reduce((sum, allocation) => sum + allocation.allocatedAmountForeign, 0);
      ...
      return {
        ...
        allocatedAmount,
        unallocatedAmount: Math.max(Number(tx.amount || 0) - allocatedAmount, 0),
        allocations,
      };
    });
    const allocatedTotal = allocationTotals.ADVANCE + allocationTotals.BALANCE + allocationTotals.COLLECTION + allocationTotals.MANUAL;
    const allocationCoveragePercent = contractTotal > 0
      ? Number(new Decimal(allocatedTotal).div(contractTotal).mul(100).toFixed(2))
      : 0;
    ...
    return {
      ...
      paymentPlan: {
        advance: {
          ...
          variance: new Decimal(receivedAdvance).minus(expectedAdvance).toNumber(),
        },
        balance: {
          ...
          allocatedTotal: allocationTotals.BALANCE + allocationTotals.COLLECTION,
          remaining: Math.max(expectedBalance - receivedBalance, 0),
          unallocated: Math.max(receivedBalance - allocationTotals.BALANCE - allocationTotals.COLLECTION, 0),
          variance: new Decimal(receivedBalance).minus(expectedBalance).toNumber(),
        },
        manual: {
      ...
      unallocatedTransactions: transactionAllocations.filter((tx) => tx.unallocatedAmount > 0),
  ...
  private deriveExpectedAdvance(contractTotal: number, proformaInvoice?: ProformaPaymentSnapshot | null) {
    ...
    if (depositPercent > 0) {
      return Number(new Decimal(contractTotal).mul(depositPercent).div(100).toFixed(2));
    }
  ...
  private async getExpectedAmountForTransaction(
    ...
    if (data.type === TradeFinanceType.TT_ADVANCE) {
      return summary.remainingAdvance > 0 ? summary.remainingAdvance : summary.openAr;
    }
    return summary.openAr;
  }

  private getReconciliationStatus(amount: number, expectedAmount: number | null) {
    ...
    if (variance.abs().lessThanOrEqualTo(0.01)) return ReconciliationStatus.MATCHED;
   ...
   */
  async createTransaction(data: CreateTradeFinanceTransactionInput, user: User) {
    const { vendorInvoiceIds, ...rest } = data;
    const invoiceIds = vendorInvoiceIds && vendorInvoiceIds.length > 0
      ? vendorInvoiceIds
    ...
    if (invoiceIds.length > 0) {
      ...
      if (invoices.length !== invoiceIds.length) {
        throw new BadRequestException('Khong tim thay day du hoa don NCC can thanh toan');
      }

      const invalidInvoice = invoices.find((invoice) => invoice.status !== VendorInvoiceStatus.PENDING);
      if (invalidInvoice) {
        throw new BadRequestException(`Hoa don ${invalidInvoice.invoiceNumber} khong o trang thai cho thanh toan`);
      }

      const currencies = new Set(invoices.map((invoice) => invoice.currency || 'VND'));
      if (currencies.size > 1) {
        throw new BadRequestException('Khong the thanh toan nhieu hoa don khac tien te trong mot giao dich');
      }

      const expectedAmount = invoices.reduce((sum, invoice) => sum + Number(invoice.totalAmount), 0);
      if (Math.abs(Number(rest.amount) - expectedAmount) > 0.01) {
        throw new BadRequestException('So tien thanh toan phai bang tong gia tri cac hoa don da chon');
      }
      ...
      for (const invoice of invoices) {
        ...
        if (!payable) {
          throw new BadRequestException(`Hoa don ${invoice.invoiceNumber} chua co cong no AP`);
        }

        if (!payable.isApprovedForPayment) {
          throw new BadRequestException(`Cong no AP cua hoa don ${invoice.invoiceNumber} chua duoc duyet thanh toan`);
        }
      }
    ...
    const reconciliationStatus = this.getReconciliationStatus(Number(rest.amount || 0), expectedAmount);
    ...
    const transaction = this.transactionRepository.create({
      ...rest,
      vendorInvoiceId: vendorInvoiceIds && vendorInvoiceIds.length > 0 ? vendorInvoiceIds[0] : rest.vendorInvoiceId,
      createdByUsername: user.username,
      status: TradeFinanceStatus.PENDING,
      expectedAmount: expectedAmount ?? undefined,
      varianceAmount: expectedAmount === null ? 0 : new Decimal(rest.amount || 0).minus(expectedAmount).toNumber(),
      reconciliationStatus,
    });
  ...
  async updateTransactionStatus(id: string, status: TradeFinanceStatus, user: User) {
    ...
    try {
      ...
      if (transaction.note && transaction.note.includes('Multi-payment for invoices:')) {
          const match = transaction.note.match(/Multi-payment for invoices: ([^.]+)/);
          if (match) {
      ...
      if (invoiceIds.length > 0) {
          transaction['invoices'] = await queryRunner.manager.find(VendorInvoice, {
              where: { _id: In(invoiceIds) },
              relations: ['vendor']
          });
      }
      ...
      if (status === TradeFinanceStatus.RECEIVED && transaction.salesContractId) {
        const expectedAmount = await this.getExpectedAmountForTransaction(transaction);
        transaction.expectedAmount = expectedAmount ?? transaction.expectedAmount;
        transaction.varianceAmount = expectedAmount === null
          ? 0
          : new Decimal(transaction.amount || 0).minus(expectedAmount).toNumber();
        transaction.reconciliationStatus = this.getReconciliationStatus(Number(transaction.amount || 0), expectedAmount);
        ...
        transaction.reconciledByUsername = user.username;
        transaction.reconciledAt = new Date();
      } else if ([TradeFinanceStatus.REJECTED, TradeFinanceStatus.CANCELLED].includes(status)) {
      ...
      if (status === TradeFinanceStatus.RECEIVED || status === TradeFinanceStatus.PAID) {
        ...
        if (invoiceIds.length > 0) {
            await queryRunner.manager.update(VendorInvoice, { _id: In(invoiceIds) }, {
            ...
            for (const invoice of invoices) {
                ...
                if (payable) {
                    await queryRunner.manager.update(AccountPayable, { _id: payable._id }, {
  ...
  private async postAccountingForTransaction(tx: TradeFinanceTransaction, manager: EntityManager) {
    ...
    if (invoices && invoices.length > 0) {
        ...
        for (const inv of invoices) {
            ...
            const bookVndValue = Number(inv.totalAmount) * Number(inv.exchangeRate || 1);
            // Tỷ giá thực tế (Lúc thanh toán)
            const actualVndValue = Number(inv.totalAmount) * Number(tx.exchangeRate);
            ...
            if (exchangeDiff.abs().greaterThan(1)) {
                if (exchangeDiff.greaterThan(0)) {
                    journalItems.push({ accountCode: '635', debit: exchangeDiff.abs().toNumber(), credit: 0 });
                } else {
                    journalItems.push({ accountCode: '515', debit: 0, credit: exchangeDiff.abs().toNumber() });
                }
            }
        }
  ...
  async findAllTransactions(query: QueryParams) {
    ...
    const [results, totalItems] = await this.transactionRepository.findAndCount({
      where: filter,
      relations: ['salesContract', 'salesContract.buyer', 'vendorInvoice', 'vendorInvoice.vendor', 'createdBy'],
      order: sort || { createdAt: 'DESC' },
      skip: skip,
      take: pageSize
    });
