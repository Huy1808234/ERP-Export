...
import {
  VatRefundDossier,
  VatRefundStatus,
} from './entities/vat-refund-dossier.entity';
...
import {
  DocumentType,
  ExportDocument,
} from '../export-documents/entities/export-document.entity';
...
type LedgerBalanceRow = {
  debit?: DecimalLike;
  credit?: DecimalLike;
  ledger_debit?: DecimalLike;
  ledger_credit?: DecimalLike;
};
...
type TotalBalanceRow = {
  totalDebit?: DecimalLike;
  totalCredit?: DecimalLike;
};

type TrendAggregateRow = AccountAggregateRow & {
  month: string;
};
...
export class AccountingService {
  ...
  private getQueryPositiveInteger(
    query: QueryParams | undefined,
    key: string,
    fallback: number,
  ): number {
    const value = Number(this.getQueryString(query, key));
  ...
  private mapExportDocumentTrace(
    document: ExportDocument,
    traceDate: dayjs.Dayjs | null,
  ): TaxDocumentTraceItem {
    return {
      _id: document._id,
  ...
  private async buildTaxDocumentTrace(
    start: dayjs.Dayjs,
    end: dayjs.Dayjs,
  ): Promise<TaxDocumentTrace> {
  ...
  private buildTaxReconciliation(params: {
    ...
    journalLines: VatJournalLineRow[];
    refundableVat: Decimal;
  }): TaxReconciliation {
    const tracedVatJournalLineCount = params.journalLines.filter((line) =>
      Boolean(line.referenceId),
    ).length;
    const untracedVatJournalLineCount =
      params.journalLines.length - tracedVatJournalLineCount;

    return {
      customsDeclarationCount: params.documentTrace.customsDeclarations.length,
      certificateOfOriginCount:
        params.documentTrace.certificatesOfOrigin.length,
      ...
      refundableVatCoveredByDossier:
        ...
        params.documentTrace.vatRefundDossiers.some((record) =>
          [
            VatRefundStatus.SUBMITTED,
            VatRefundStatus.APPROVED,
            VatRefundStatus.PAID,
  ...
  private getErrorMessage(error: unknown) {
    ...
    return 'Unexpected accounting error';
  }

  private toDecimalValue(value: DecimalLike) {
  ...
  private buildPeriodHash(
    period: AccountingPeriod,
    extra: AccountingAuditPayload = {},
  ) {
  ...
  private async findExactPeriod(
    ...
    return manager
      ...
      .andWhere('period.endDate = :endDate', {
        endDate: endDate.format('YYYY-MM-DD'),
      })
      .getOne();
  }

  private async assertNoPreviousOpenPeriods(
    period: AccountingPeriod,
    manager: EntityManager,
  ) {
    const previousOpen = await manager
      .getRepository(AccountingPeriod)
      .createQueryBuilder('candidate')
      .where('candidate.status = :status', {
        status: AccountingPeriodStatus.OPEN,
      })
      .andWhere('candidate.endDate < :startDate', {
        startDate: this.toDateOnly(period.startDate),
      })
    ...
    if (previousOpen) {
      throw new BadRequestException(
        'Close earlier open accounting periods before closing this period.',
      );
    }
  }

  private async assertNoLaterClosedOrLockedPeriods(
    period: AccountingPeriod,
    manager: EntityManager,
  ) {
    const laterPeriod = await manager
      .getRepository(AccountingPeriod)
      .createQueryBuilder('candidate')
      ...
      .andWhere('candidate.startDate > :endDate', {
        endDate: this.toDateOnly(period.endDate),
      })
    ...
    if (laterPeriod) {
      throw new BadRequestException(
        'Cannot reopen a period while later periods are closed or locked.',
      );
    }
  }
  ...
  private validateJournalItems(items: JournalItemInput[]) {
    ...
    const normalizedItems = items.map((item, index) => {
      if (!item.accountCode || !/^[0-9]{3,4}$/.test(String(item.accountCode))) {
        throw new BadRequestException(
          `Invalid account code at line ${index + 1}`,
        );
      }

      const debit = this.normalizeMoney(item.debit, `Line ${index + 1} debit`);
      const credit = this.normalizeMoney(
        item.credit,
        `Line ${index + 1} credit`,
      );
      ...
      if (debit.equals(0) && credit.equals(0)) {
        throw new BadRequestException(
          `Line ${index + 1} must have either debit or credit`,
        );
      }
  ...
  async createJournalEntry(
    ...
    const postingPeriod =
      await this.accountingPeriodGuardService.assertPostingAllowed(
        {
          entryDate,
          referenceType: data.referenceType || null,
    ...
    const execute = async (mgr: EntityManager) => {
      ...
      const ledgerEntries = normalizedItems.map((item) =>
        mgr.create(LedgerEntry, {
          ...item,
      ...
      const partnerIds = [
        ...new Set(ledgerEntries.map((e) => e.partnerId).filter(Boolean)),
      ];
      for (const pid of partnerIds) {
        await this.syncPartnerBalance(pid, mgr);
      }
      ...
      await this.appendAccountingAuditEvent(
        {
          ...
          payload: {
            ...
            description: data.description,
            totalDebit: normalizedItems.reduce(
  ...
  async deleteJournalEntriesByReference(
    refType: string,
    refId: string,
    ...
    if (descriptionPattern) {
      qb.andWhere('je.description LIKE :pattern', {
    ...
    try {
      for (const entry of entries) {
        const existingReversal = await queryRunner.manager
        ...
        await this.appendAccountingAuditEvent(
          {
            ...
            payload: {
              ...
              reason:
                'Immutable reversal generated instead of deleting posted accounting records',
            },
          },
          queryRunner.manager,
        );
  ...
  private async syncPartnerBalance(partnerId: string, manager: EntityManager) {
    ...
    const arResult = await manager
      ...
      .getRawOne<LedgerBalanceRow>();

    // TECH LEAD FIX: Handle both alias styles (debit vs ledger_debit) and ensure numeric conversion
    const arDebit = this.toDecimalValue(
      arResult?.debit ?? arResult?.ledger_debit,
    );
    const arCredit = this.toDecimalValue(
      arResult?.credit ?? arResult?.ledger_credit,
    );
    let arBalance = arDebit.minus(arCredit);

    // Calculate AP (331)
    const apResult = await manager
      .createQueryBuilder(LedgerEntry, 'ledger')
      .innerJoin('ledger.journalEntry', 'je')
      .select('SUM(ledger.debit)', 'debit')
      .addSelect('SUM(ledger.credit)', 'credit')
    ...
    const apDebit = this.toDecimalValue(
      apResult?.debit ?? apResult?.ledger_debit,
    );
    const apCredit = this.toDecimalValue(
      apResult?.credit ?? apResult?.ledger_credit,
    );
    let apBalance = apCredit.minus(apDebit);

    // Currency Conversion if partner uses USD
    if (partner.defaultCurrency === 'USD') {
      // Use the latest USD/VND transfer rate from the currencies table so that
      // partner balances stay in sync with daily exchange rate updates.
      let exRate: number;
      try {
        exRate = await this.currenciesService.getLatestVndRate(
          'USD',
  ...
  async findAllJournal(query: QueryParams = {}) {
    const current = this.getQueryPositiveInteger(query, 'current', 1);
    const pageSize = this.getQueryPositiveInteger(query, 'pageSize', 20);
    const startDate = this.getQueryString(query, 'startDate');
    const endDate = this.getQueryString(query, 'endDate');
    const skip = (current - 1) * pageSize;

    const where: FindOptionsWhere<JournalEntry> = {};
    const status = this.getQueryString(query, 'status');
    const referenceType = this.getQueryString(query, 'referenceType');
    const referenceId = this.getQueryString(query, 'referenceId');
    const entryNumber = this.getQueryString(query, 'entryNumber');
    const createdByUsername = this.getQueryString(query, 'createdByUsername');

    if (this.isEnumValue(JournalStatus, status)) where.status = status;
  ...
  async getAccountBalance(accountCode: string) {
    const result = await this.ledgerRepository
      ...
      .andWhere('je.status = :status', { status: JournalStatus.POSTED })
      .getRawOne<TotalBalanceRow>();

    const debit = this.toDecimalValue(result?.totalDebit);
    const credit = this.toDecimalValue(result?.totalCredit);
  ...
  async getSummaryReport(query?: DateRangeQuery) {
    const current = await this.calculateSummary(
      query?.startDate,
      query?.endDate,
    );
  ...
  private async calculateSummary(startDate?: string, endDate?: string) {
    ...
    const results = await qb
      ...
      .getRawMany<AccountAggregateRow>();
    ...
    results.forEach((row) => {
      const code = row.code;
      const debit = this.toDecimalValue(row.debit);
      const credit = this.toDecimalValue(row.credit);
  ...
  async getTrendReport(query?: DateRangeQuery) {
    ...
    const monthlyData: Record<string, TrendSummaryRow> = {};
  ...
  async getOverdueAging() {
    ...
    const payables = await this.dataSource
    ...
    payables.forEach((ap) => {
      ...
      const amount = Math.max(
        Number(ap.amount || 0) - Number(ap.paidAmount || 0),
        0,
      );
      if (amount <= 0) return;
  ...
  async getARAging() {
    const receivableRepository =
      this.dataSource.getRepository(AccountReceivable);
    ...
    const openDebitsByPartner = new Map<
      string,
      Array<{ entryDate: Date; remaining: Decimal }>
    >();
    ...
    for (const openDebits of openDebitsByPartner.values()) {
      for (const debitLine of openDebits) {
        ...
        const diffDays = Math.floor(
          (today.getTime() - new Date(debitLine.entryDate).getTime()) /
            (1000 * 3600 * 24),
        );
  ...
  async getCashFlowReport(query?: DateRangeQuery) {
    const startDate = query?.startDate
      ? new Date(query.startDate)
    ...
    const results = await this.ledgerRepository
      ...
      .andWhere('je.status = :status', { status: JournalStatus.POSTED })
      .andWhere(
        '(ledger.accountCode LIKE :cash1 OR ledger.accountCode LIKE :cash2)',
        { cash1: '111%', cash2: '112%' },
      )
      .groupBy('ledger.accountCode')
      .getRawMany<AccountAggregateRow>();
    ...
    results.forEach((row) => {
      inflows += this.toDecimalValue(row.debit).toNumber();
      outflows += this.toDecimalValue(row.credit).toNumber();
    });
  ...
  async getFinancialRatios(query?: DateRangeQuery) {
    const balance = await this.getBalanceSheet(query);
    const summary = await this.getSummaryReport(query);

    const currentAssets = balance.assets
      .filter((a) => ['111', '112', '131', '152', '156'].includes(a.code))
      .reduce((s, i) => s + i.balance, 0);
    const currentLiabilities = balance.liabilities
      .filter((l) => ['331', '333'].includes(l.code))
      .reduce((s, i) => s + i.balance, 0);
    ...
    return {
      ...
      grossMargin:
        summary.current.revenue > 0
          ? (summary.current.revenue - summary.current.cogs) /
            summary.current.revenue
          : 0,
      netMargin:
        summary.current.revenue > 0
          ? summary.current.netProfit / summary.current.revenue
          : 0,
      inventoryTurnover:
        summary.current.cogs > 0
          ? summary.current.cogs / (currentAssets * 0.5 || 1)
          : 0,
    };
  }

  async getBalanceSheet(query?: DateRangeQuery) {
    const qb = this.ledgerRepository
      .createQueryBuilder('ledger')
      .innerJoin('ledger.journalEntry', 'je')
      .select('ledger.accountCode', 'code')
    ...
    equity.push({
      code: '421',
      name: this.getAccountName('421'),
      balance: summary.current.netProfit,
    });

    return { assets, liabilities, equity };
  }

  async getVatReport(query?: DateRangeQuery) {
    ...
    const journalLines = await this.ledgerRepository
      ...
      .getRawMany<VatJournalLineRow>();

    const inputVat = results
      .filter((r) => ['133', '1331'].includes(r.code))
      .reduce(
        (sum, row) =>
          sum.plus(
            this.toDecimalValue(row.debit).minus(
              this.toDecimalValue(row.credit),
            ),
          ),
        new Decimal(0),
      );
    const outputVat = results.find((r) => r.code === '3331');
    const outputVatAmount = this.toDecimalValue(outputVat?.credit).minus(
      this.toDecimalValue(outputVat?.debit),
    );
    ...
    const exportRevenueVnd = results
      ...
      .reduce(
        (sum, row) =>
          sum.plus(
            this.toDecimalValue(row.credit).minus(
              this.toDecimalValue(row.debit),
            ),
          ),
        new Decimal(0),
      );
    ...
    const taxPeriod = await this.accountingPeriodRepository
      ...
      .andWhere('period.endDate >= :startDate', {
        startDate: start.format('YYYY-MM-DD'),
      })
    ...
    const warnings = [
      ...(!taxPeriod
        ? ['No accounting period is configured for this tax report range.']
        : []),
      ...(exportRevenueVnd.greaterThan(0) &&
      reconciliation.customsDeclarationCount === 0
        ? [
            'Export revenue exists but no customs declaration was traced for this tax period.',
          ]
        : []),
      ...(exportRevenueVnd.greaterThan(0) &&
      reconciliation.certificateOfOriginCount === 0
        ? [
            'Export revenue exists but no C/O document was traced for this tax period.',
          ]
        : []),
      ...(refundableVat.greaterThan(0) &&
      !reconciliation.refundableVatCoveredByDossier
        ? [
      ...
      ...journalLines
        .filter(
          (line) =>
            ['133', '1331', '3331'].includes(String(line.accountCode)) &&
            !line.referenceId,
        )
        .map(
          (line) =>
            `VAT line ${line.entryNumber || line.journalEntryId} is missing referenceId.`,
        ),
    ];
    const accountBreakdown = results.map((r) => ({
      ...
      net: ['3331', '511', '3387'].includes(String(r.code))
        ? this.toDecimalValue(r.credit)
            .minus(this.toDecimalValue(r.debit))
            .toNumber()
        : this.toDecimalValue(r.debit)
            .minus(this.toDecimalValue(r.credit))
            .toNumber(),
    }));
    const summary = {
      ...
      taxableRevenueVnd: outputVatAmount.greaterThan(0)
        ? outputVatAmount.dividedBy(0.1).toDecimalPlaces(2).toNumber()
        : 0,
    };
  ...
  /**
   * Xß╗¡ l├╜ ch├¬nh lß╗çch tß╗╖ gi├í (Mß╗Ñc 10 PRD)
   * Realized Gain/Loss khi thu/thanh to├ín ngoß║íi tß╗ç
   */
  async exportTaxReportCsv(query?: { startDate?: string; endDate?: string }) {
    ...
    const rows: unknown[][] = [
      ...
      [
        ...
        report.period.accountingPeriodId,
        report.period.accountingPeriodStatus,
      ],
    ...
    rows.push([
      ...
      report.reconciliation.customsDeclarationCount,
    ]);
    rows.push([
      'reconciliation',
      'certificateOfOriginCount',
      report.reconciliation.certificateOfOriginCount,
    ]);
    rows.push([
      'reconciliation',
      'vatRefundDossierCount',
      report.reconciliation.vatRefundDossierCount,
    ]);
    rows.push([
      'reconciliation',
      'untracedVatJournalLineCount',
      report.reconciliation.untracedVatJournalLineCount,
    ]);
  ...
  async processExchangeGainLoss(
    ...
    if (diff.greaterThan(0)) {
      // L├úi tß╗╖ gi├í (Gain): Nß╗ú 131/112, C├│ 515
      items.push({
        accountCode: '515',
        debit: 0,
        credit: diff.abs().toNumber(),
      });
      items.push({
        accountCode: '131',
        debit: diff.abs().toNumber(),
        credit: 0,
        partnerId: data.partnerId,
      });
    } else {
      // Lß╗ù tß╗╖ gi├í (Loss): Nß╗ú 635, C├│ 131/112
      items.push({
        accountCode: '635',
        debit: diff.abs().toNumber(),
        credit: 0,
  ...
  private async buildClosingItems(
    startDate: Date,
    endDate: Date,
    manager: EntityManager,
  ) {
    const rows = await manager
      ...
      .andWhere('je.status = :status', { status: JournalStatus.POSTED })
      .andWhere(
        '(ledger.accountCode LIKE :revenuePrefix OR ledger.accountCode LIKE :expensePrefix)',
        {
          revenuePrefix: '5%',
          expensePrefix: '6%',
        },
  ...
  async findPeriods(query: QueryParams = {}) {
    const current = this.getQueryPositiveInteger(query, 'current', 1);
    const pageSize = this.getQueryPositiveInteger(query, 'pageSize', 20);
    const status = this.getQueryString(query, 'status');
    const where: FindOptionsWhere<AccountingPeriod> = {};
    if (this.isEnumValue(AccountingPeriodStatus, status)) {
      where.status = status;
    }
  ...
  async openPeriod(
    ...
    if (overlappingPeriod) {
      throw new BadRequestException(
        'Ky ke toan bi trung/chong len ky da ton tai',
      );
    }
  ...
  async findPeriodClosePackets(
    ...
    return this.accountingClosePolicyService.findPeriodClosePackets(
      recordId,
      query,
    );
  }
  ...
  async closePeriod(
    ...
    try {
      ...
      const periodRepository =
        queryRunner.manager.getRepository(AccountingPeriod);
      const period = await this.findExactPeriod(
        start,
        end,
        queryRunner.manager,
      );
      if (!period)
        throw new BadRequestException(
          'Open the accounting period before closing it.',
        );
      ...
      const items = await this.buildClosingItems(
        start.toDate(),
        end.toDate(),
        queryRunner.manager,
      );
      ...
      const closePolicy =
        await this.accountingClosePolicyService.buildClosePolicy(
          period,
          taxReport,
          items,
          queryRunner.manager,
      ...
      if (!closePolicy.canClose) {
        const failedLabels = closePolicy.checklist
          ...
          .join(', ');
        throw new BadRequestException(
          `Accounting close policy failed: ${failedLabels}`,
        );
      }

      const taxReportRun =
        await this.accountingClosePolicyService.createTaxReportRun(
          period,
          taxReport,
          username,
          queryRunner.manager,
      ...
      const closePacket =
        await this.accountingClosePolicyService.createClosePacket(
          savedPeriod,
          taxReportRun,
          closingEntry,
          closePolicy,
  ...
  async reopenPeriod(
    ...
    if (!reason || reason.length < 10) {
      throw new BadRequestException(
        'Can co ly do mo lai ky ke toan toi thieu 10 ky tu',
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (period.status === AccountingPeriodStatus.LOCKED)
        throw new BadRequestException(
          'Ky da lock vinh vien, khong duoc mo lai',
        );
      if (period.status !== AccountingPeriodStatus.CLOSED)
        throw new BadRequestException('Chi ky da dong moi can mo lai');
      if (Number(period.reopenCount || 0) >= 3)
        throw new BadRequestException(
          'Ky ke toan da vuot qua so lan mo lai toi da',
        );
      if (period.reopenApprovalWorkflowRequest_id) {
        throw new BadRequestException(
          'Ky ke toan da co yeu cau mo lai dang cho duyet',
        );
      }
      await this.assertNoLaterClosedOrLockedPeriods(period, manager);

      const closePacket =
        await this.accountingClosePolicyService.findLatestClosePacket(recordId);
      if (!closePacket) {
        throw new BadRequestException(
          'Can co close packet bat bien truoc khi gui yeu cau mo lai ky',
        );
      }
      const auditVerification = await this.verifyAuditChain();
      if (!auditVerification.valid) {
        throw new BadRequestException(
          'Accounting audit hash chain khong hop le; khong duoc mo lai ky',
        );
      }
      ...
      if (!matchingRule) {
        throw new BadRequestException(
          'Chua cau hinh approval matrix cho mo lai ky ke toan',
        );
      }
  ...
  async completePeriodReopenWorkflowApproval(
    ...
    const approvedReason =
      reason?.trim() || 'Approved accounting period reopen';
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!period) throw new BadRequestException('Accounting period not found');
      if (
        period.reopenApprovalWorkflowRequest_id &&
        period.reopenApprovalWorkflowRequest_id !== approvalWorkflowRequest_id
      ) {
        throw new BadRequestException(
          'Approval request does not match accounting period reopen request',
        );
      }
      if (period.status === AccountingPeriodStatus.LOCKED)
        throw new BadRequestException(
  ...
  async lockPeriod(
    ...
    if (!reason || reason.length < 10) {
      throw new BadRequestException(
        'Can co ly do lock ky ke toan toi thieu 10 ky tu',
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (period.status !== AccountingPeriodStatus.CLOSED) {
        throw new BadRequestException(
          'Only CLOSED periods can be locked permanently.',
        );
      }
      if (period.lockApprovalWorkflowRequest_id) {
        throw new BadRequestException(
          'Ky ke toan da co yeu cau lock dang cho duyet',
        );
      }
      ...
      if (!closePacket) {
        throw new BadRequestException(
          'A frozen close packet is required before permanently locking an accounting period.',
        );
      }
      const auditVerification = await this.verifyAuditChain();
      if (!auditVerification.valid) {
        throw new BadRequestException(
          'Accounting audit hash chain must be valid before locking the period.',
        );
      }
      ...
      if (!matchingRule) {
        throw new BadRequestException(
          'Chua cau hinh approval matrix cho lock ky ke toan',
        );
      }
  ...
  async completePeriodLockWorkflowApproval(
    ...
    const approvedReason =
      reason?.trim() || 'Approved permanent accounting period lock';
    ...
    if (!period) throw new BadRequestException('Accounting period not found');
    if (
      period.lockApprovalWorkflowRequest_id &&
      period.lockApprovalWorkflowRequest_id !== approvalWorkflowRequest_id
    ) {
      throw new BadRequestException(
        'Approval request does not match accounting period lock request',
      );
    }
    if (period.status !== AccountingPeriodStatus.CLOSED) {
      throw new BadRequestException(
  ...
  async findFxRevaluations(query: QueryParams = {}) {
    const current = this.getQueryPositiveInteger(query, 'current', 1);
    const pageSize = this.getQueryPositiveInteger(query, 'pageSize', 20);
    const periodId = this.getQueryString(query, 'periodId');
    const currency = this.getQueryString(query, 'currency');
    const status = this.getQueryString(query, 'status');
    const where: FindOptionsWhere<FxRevaluation> = {};
    if (periodId) where.periodId = periodId;
    if (currency) where.currency = String(currency).toUpperCase();
    if (this.isEnumValue(FxRevaluationStatus, status)) where.status = status;
  ...
  async runUnrealizedFxRevaluation(
    dto: RunFxRevaluationDto,
    user?: AuthenticatedUser,
  ) {
    ...
    if (!revaluationDate.isValid())
      throw new BadRequestException('Ngay danh gia lai khong hop le');
    if (closingRate.lessThanOrEqualTo(0))
      throw new BadRequestException('Ty gia cuoi ky phai lon hon 0');
    ...
    try {
      ...
      if (sourceType === FxRevaluationSourceType.AR) {
        const receivables = await queryRunner.manager
        ...
        for (const receivable of receivables) {
          const openForeign = new Decimal(receivable.amountForeign || 0).minus(
            receivable.paidAmountForeign || 0,
          );
          if (openForeign.lessThanOrEqualTo(0)) continue;

          const bookValue = new Decimal(receivable.amountVnd || 0).minus(
            receivable.paidAmountVnd || 0,
          );
          ...
          records.push(
            fxRepository.create({
              ...
              status: dto.postJournal
                ? FxRevaluationStatus.POSTED
                : FxRevaluationStatus.DRAFT,
      ...
      const savedRecords = records.length
        ? await fxRepository.save(records)
        : [];
      const netGainLoss = savedRecords.reduce(
        (sum, item) => sum.plus(item.gainLossVnd || 0),
        new Decimal(0),
      );
      ...
      await this.appendAccountingAuditEvent(
        {
          eventType: dto.postJournal
            ? 'FX_REVALUATION_POSTED'
            : 'FX_REVALUATION_DRAFTED',
  ...
  async findVatRefundDossiers(query: QueryParams = {}) {
    const current = this.getQueryPositiveInteger(query, 'current', 1);
    const pageSize = this.getQueryPositiveInteger(query, 'pageSize', 20);
    const status = this.getQueryString(query, 'status');
    const where: FindOptionsWhere<VatRefundDossier> = {};
    if (this.isEnumValue(VatRefundStatus, status)) where.status = status;
  ...
  async createVatRefundDossier(
    dto: CreateVatRefundDossierDto,
    user?: AuthenticatedUser,
  ) {
    ...
    const outputVat = new Decimal(
      dto.outputVatAmount ?? vatReport.outputVat ?? 0,
    );
    const defaultRefund = Decimal.max(inputVat.minus(outputVat), 0);
    const refundAmount = new Decimal(
      dto.refundAmount ?? defaultRefund.toNumber(),
    );
    ...
    const dossier = this.vatRefundRepository.create({
      ...
      exportRevenueVnd: Number(
        dto.exportRevenueVnd ?? vatReport.exportRevenueVnd ?? 0,
      ),
  ...
  async submitVatRefund(recordId: string, user?: AuthenticatedUser) {
    const dossier = await this.vatRefundRepository.findOne({
      where: { _id: recordId },
    });
    if (!dossier) throw new BadRequestException('VAT refund dossier not found');
    if (
      ![VatRefundStatus.DRAFT, VatRefundStatus.REJECTED].includes(
        dossier.status,
      )
    ) {
    ...
    const entry = await this.createJournalEntry(
      {
        ...
        items: [
          {
            accountCode: '1388',
            debit: Number(dossier.refundAmount || 0),
            credit: 0,
          },
          {
            accountCode: '1331',
            debit: 0,
            credit: Number(dossier.refundAmount || 0),
          },
        ],
      },
      manager,
  ...
  async payVatRefund(
    ...
    if (dossier.status !== VatRefundStatus.APPROVED) {
      throw new BadRequestException(
        'Chi ho so da duyet moi duoc ghi nhan nhan tien',
      );
    }
  ...
  async findAuditEvents(query: QueryParams = {}) {
    const current = this.getQueryPositiveInteger(query, 'current', 1);
    const pageSize = this.getQueryPositiveInteger(query, 'pageSize', 50);
    const entityType = this.getQueryString(query, 'entityType');
    const entityId = this.getQueryString(query, 'entityId');
    const eventType = this.getQueryString(query, 'eventType');
    const referenceType = this.getQueryString(query, 'referenceType');
    const referenceId = this.getQueryString(query, 'referenceId');
    const where: FindOptionsWhere<AccountingAuditEvent> = {};
  ...
  getShipmentProfitability(shipmentId: string) {
    return {
      shipmentId,
      revenue: 0,
      cogs: 0,
  ...
  getPLDrilldown(query: QueryParams) {
  ...
  getARAgingDetails(query: QueryParams) {
    const current = Number(query?.current || 1);
    const pageSize = Number(query?.pageSize || 20);
    return {
      results: [],
  ...
  getAPAgingDetails(query: QueryParams) {
    const current = Number(query?.current || 1);
    const pageSize = Number(query?.pageSize || 20);
    return {
      results: [],
  ...
  private getAccountName(code: string): string {
    const map: Record<string, string> = {
      '111': 'Tiß╗ün mß║╖t',
      '112': 'Tiß╗ün gß╗¡i ng├ón h├áng',
      '131': 'Phß║úi thu kh├ích h├áng (AR)',
      '133': 'Thuß║┐ GTGT ─æ╞░ß╗úc khß║Ñu trß╗½ (─Éß║ºu v├áo)',
      '1331': 'Thuß║┐ GTGT ─æ╞░ß╗úc khß║Ñu trß╗½ h├áng h├│a dß╗ïch vß╗Ñ',
      '1388': 'Phß║úi thu kh├íc',
      '156': 'H├áng h├│a tß╗ôn kho',
      '331': 'Phß║úi trß║ú ng╞░ß╗¥i b├ín (AP)',
      '333': 'Thuß║┐ v├á c├íc khoß║ún phß║úi nß╗Öp',
      '3331': 'Thuß║┐ GTGT phß║úi nß╗Öp (─Éß║ºu ra)',
      '3387': 'Doanh thu ch╞░a thß╗▒c hiß╗çn (In-Transit)',
      '3388': 'Phß║úi trß║ú, phß║úi nß╗Öp kh├íc',
      '413': 'Ch├¬nh lß╗çch tß╗╖ gi├í hß╗æi ─æo├íi',
      '411': 'Vß╗æn g├│p chß╗º sß╗ƒ hß╗»u',
      '421': 'Lß╗úi nhuß║¡n sau thuß║┐ ch╞░a ph├ón phß╗æi',
      '511': 'Doanh thu b├ín h├áng',
      '515': 'Doanh thu hoß║ít ─æß╗Öng t├ái ch├¡nh',
      '632': 'Gi├í vß╗æn h├áng b├ín',
      '635': 'Chi ph├¡ t├ái ch├¡nh',
      '642': 'Chi ph├¡ quß║ún l├╜ doanh nghiß╗çp',
    };
